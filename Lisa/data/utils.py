...
`unit_to_attr_map` is a dictionary that maps physical units to inovesa attributes.
...
`unit_to_attr_map_v15` for versions 15 and up (until the next change in inovesa is made).

`attr_to_unit_map` and `attr_to_unit_map_v15` are just reversed dictionaries to make it possible
to find the corresponding readable/printable unit for an attribute

Then `alias_map` is generated that makes it possible to not only use the exact specification
used in unit_to_attr_map but also the ones specified in `alias_map`. The reason for this is
that the user does not have to remember the exact unit string.

Then a `complete_unit_map` (and `complete_unit_map_v15`) is generated where all the entries in
alias_map are mapped to attributes
...
class InovesaVersion(object):
    ...
    def __init__(self, release, minor, fix):
        ...
        self.minor = minor
        self.fix = -fix if fix < 0 else fix
        self._v_tup = (self.release,
                       self.minor,
                       self.fix if self.fix > 0 else 100)
...
version9_1 = InovesaVersion(0, 9, 1)
...
attr_to_unit_map_v9 = {v: k for k, v in unit_to_attr_map_v9.items() if k != "ts"}
...
# also add the unit it self as alias
inverse_alias_map.update({k.lower(): k for k in alias_map.keys()})

# add cps etc for unit look up
cpsetc = {"cps": "C/s", "aps": "A/s", "cpev": "C/eV", "apev": "A/eV",
...
# NOTE: cps etc is not handled here but in Data directly as it consists of a calculation
# other than just reading the factor from the file


def _build_complete_unit_map(version):
    unit_map = {}
    if version == version9_1:
        u_t_a_m = unit_to_attr_map_v9
    elif version < version15_1:
        u_t_a_m = unit_to_attr_map
    else:
        u_t_a_m = unit_to_attr_map_v15
...
complete_unit_map_v9 = _build_complete_unit_map(version9_1)
...
def attr_from_unit(unit, version):
    ...
    try:
        if version == version9_1:
            return complete_unit_map_v9[unit.lower()]
...
def color_map_inferno():
    ...
    return ListedColormap(_inferno_data, name='inferno')
