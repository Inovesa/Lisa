...
def plot(func):
    def decorated(*args, **kwargs):
        ...
        elif ("fig" in kwargs and isinstance(kwargs["fig"], plt.Figure)):
            fig = kwargs["fig"]
            ax = fig.add_subplot(111)
        else:
            fig = plt.figure()
...
#            ax.plot(x, y, label=kwargs["label"], color=colors[nop]) # do not use colors use default ones
            ax.plot(x, y, label=kwargs["label"], alpha=(trans_value if nop > 0 else 1))
            ax.legend(loc="best")
        else:
#            ax.plot(x, y, color=colors[nop]) # do not use colors use default ones
            ax.plot(x, y, alpha=(trans_value if nop > 0 else 1))
